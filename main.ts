...
class Student {
    ...
    courses: string[];
    balance: number;
    feeDueDates: { [month: string]: number };
    ...
    pay_fees(amount: number, month: string) {
        if (amount > 0) {
            this.balance -= amount;
...
class Student_manager {
    students: Student[];
    feeCollection: { [month: string]: number };
    dueDates: { [month: string]: Date };
    ...
    // Method to add balance to student account
    add_student_balance(student_id: number, amount: number) {
        let student = this.find_student(student_id);
        if (student) {
    ...
    // Method to pay student fees
    pay_student_fees(student_id: number, amount: number, month: string) {
        let student = this.find_student(student_id);
        if (student) {
    ...
    record_fee_collection(amount: number, month: string) {
